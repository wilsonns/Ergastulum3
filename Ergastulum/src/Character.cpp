...
Character::Character(sf::String name, sf::Vector2i pos, Spritesheet* spritesheet)
{
...
Character::Character(const json& file, sf::String mob, sf::Vector2i pos)
{
	...
	this->m_pos = pos;
	this->m_animation = std::make_unique<AnimationComponent>(m_engine->spritesheet(file.at(mob).at("name")),m_spriteSize, 0.5f);
	/*
...
SkillComponent Character::skills()
{
	return *m_skills.get();
}
