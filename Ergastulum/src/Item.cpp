...
Item::Item(const json& file, sf::String name, sf::Vector2i pos)
{
	...
	this->m_pos = pos;
	this->m_sprite = m_engine->sprite(0, name);
...
bool Healing::use(Character*owner)
{
	owner->currentResource("Health", this->attribute("Heal"));
	m_engine->gui()->menu("gameInterface")->addMessage(owner->name() + " eats the " + this->name() + " and heals " + std::to_string(this->attribute("Heal")),
