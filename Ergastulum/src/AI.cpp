...
void PlayerAI::update(Character* owner)
{
	...
	if (m_engine->gameState(m_engine->CURRENT) == m_engine->PLAYER_TURN || m_engine->gameState(m_engine->CURRENT) == m_engine->GAME_START)
	{
		...
		{
			m_engine->setGameState(m_engine->CHARACTER_MENU);
		}
	}
	else if (m_engine->gameState(m_engine->CURRENT) == m_engine->INVENTORY_MENU || m_engine->gameState(m_engine->CURRENT) == m_engine->CHARACTER_MENU)
	{
		...
		{
			m_engine->setGameState(m_engine->gameState(m_engine->PREVIOUS));
		}
		else
		{
			if (m_engine->handler()->isKeyPressed())
			{
				char choice = m_engine->handler()->getKeyChar();
				int realChoice = (int)choice - 97;
				std::string text = " pressed.";
				std::string::iterator it;
				it = text.begin();
				text.insert(it, choice);
				m_engine->gui()->menu("gameInterface")->addMessage(sf::String(text), m_engine->gui()->font("Arial"), Color::Black, Color::Black);
				std::cout << std::endl << text << std::endl;
				owner->inventory()->contents()->at(realChoice)->use(owner);
				// 
				//PENDING	
			}
		}
