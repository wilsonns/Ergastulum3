	...
	, sf::Color outlineColor, sf::RenderTarget* window,Engine*engine)
{
	...
	m_menus.insert(std::make_pair("gameInterface", std::make_unique<GameInterface>(gameInterface,messageBox,innerColor, outlineColor, 1.f,999)));
	...
	m_menus.insert(std::make_pair("characterMenu", std::make_unique<CharacterMenu>(inventory, innerColor, outlineColor, 1.f, m_engine->CHARACTER_MENU)));
	m_menus.insert(std::make_pair("mainMenu", std::make_unique<MainMenu>(sf::Sprite(m_engine->m_startScreen))));
	m_menus.at("mainMenu").get()->addButton(sf::Text(sf::String("Play"),*font("Arial"),25), sf::Vector2f(window->getSize().x/2, window->getSize().y / 2), sf::Vector2f(100.f, 50.f),
		Color::Blue, Color::White, Color::Black, 1.f,m_engine,static_cast<int>(Engine::GAME_START));
	m_menus.at("inventory").get()->addButton(sf::Text(sf::String("X"), *font("Arial"), 25), sf::Vector2f(50.f, 50.f), sf::Vector2f(25.f, 25.f),
		Color::Red, Color::White, Color::Black, 1.f, m_engine, static_cast<int>(Engine::PLAYER_TURN));
}
