...
{
	m_buttons.push_back(std::make_unique<Button>(text, size, pos, innerColor, outlineColor, textColor, outlineThickness, engine, gameState));
}
...
void Menu::update()
{
	if (m_engine->gameState(m_engine->CURRENT) == m_stateToDraw || m_stateToDraw == m_engine->ANY)
	{
		...
		{
			button.get()->update(engine()->handler(), sf::Mouse::Button::Left);
		}
	}
}
